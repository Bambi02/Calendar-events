...
async function fetchEvents() {
	try {
		...
		const eventData = data
			.map((event) => {
				if (event.day + event.month + event.year == aktualnyDatum) {
					return `<div class="event" id=${event._id}>
        ...
        <div class="icons">
			...
			.join('');
		bottom.innerHTML = eventData;
	} catch (error) {
		bottom.innerHTML = 'There is a problem, try again later';
	}
}
...
function getPocetDni() {
	...
	if (zvolenyMesiac == 1 && leapYear(zvolenyRok)) {
		populateDays(29);
	} else if (zvolenyMesiac == 1 && !leapYear(zvolenyRok)) {
		populateDays(28);
	} else {
		populateDays(pocetDniMesiaca[zvolenyMesiac]);
	}
