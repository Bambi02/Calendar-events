...
const nazovMesiaca = ["Január", "Február", "Marec", "Apríl", "Máj", "Jún", "Júl", "August", "September", "Október", "November", "December"]
...
let aktualnyDatum =dnesnyDen.toString() + (dnesnyMesiac+1).toString() + dnesnyRok.toString();
let novyDatum = zvolenyDen.toString() + (zvolenyMesiac+1).toString() + zvolenyRok.toString();
...
denVKalendari.forEach((day) => addEventListener("click", (element) => zvolDen(day, element)))
...
function createTodo () {
  ...
  if (eventName == "" || eventDesc =="" || eventTime ==""){
    ...
    document.querySelector(".add_window").insertBefore(div, document.querySelector("#name"));
...
function fetchEvents () {
  ...
  storedEvents.forEach (event => {
    bottom.innerHTML += `<div class="event">
                    ...
                    <div class="event_time"><p>${event.time[0]+event.time[1]}:${event.time[2]+event.time[3]}</p></div>
                    </div>
                    <div class="icons">
...
function deleteFromStorage (event) {
  const name = event.target.parentElement.previousElementSibling.firstElementChild.innerText;
  const desc = event.target.parentElement.previousElementSibling.firstElementChild.nextElementSibling.innerText;
...
function vytvorNovyDatum() {
  novyDatum = zvolenyDen.toString() + (zvolenyMesiac+1).toString() + zvolenyRok.toString();
}

function vytvorAktualnyDatum() {
  aktualnyDatum = zvolenyDen.toString() + (zvolenyMesiac+1).toString() + zvolenyRok.toString();
}

function aktualizujZobrazenyDatum() {
  if(dnesnyDen.toString() + (dnesnyMesiac+1).toString() + dnesnyRok.toString() === aktualnyDatum){
    selectedDay.innerText = "Dnes";
  }else if(zvolenyDen<10 && zvolenyMesiac<10){
    selectedDay.innerText = `0${zvolenyDen}.0${zvolenyMesiac+1}.${zvolenyRok} `
  }else if(zvolenyDen<10){
    selectedDay.innerText = `0${zvolenyDen}.${zvolenyMesiac+1}.${zvolenyRok} `
  }else if(zvolenyMesiac<10){
    selectedDay.innerText = `${zvolenyDen}.0${zvolenyMesiac+1}.${zvolenyRok} `
  }else{
